...
public class beste_resultat extends Sporring{
	...
	public void run(){
		...
		try {
			...
			ResultSet rs = s.executeQuery("select if(Mål.opptil = true, max(Resultat.verdi), min(Resultat.verdi)) as beste_resultat from Resultat natural join Øvelse inner join Mål on (Øvelse.øvingsID = Mål.øvingsID) where Resultat.typ = '" + type + "' and Øvelse.øvingsID = '" + øvingsid + "' and Resultat.typ = Mål.typ;");
