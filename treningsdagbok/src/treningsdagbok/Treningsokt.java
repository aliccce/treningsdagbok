...
public class Treningsokt {
	...
	public void printØvelser(){
		try{
			Statement stmt = myCon.createStatement();
			ArrayList<Integer> printedØvelser = new ArrayList<Integer>();
			...
			for (int øvingsID: øvelser){
				...
				if(result.first()){
					...
					printedØvelser.add(øvingsID);
				}
			}
			for (int gruppeID: grupper){
				ResultSet result = stmt.executeQuery("select øvingsid as id, øvingsnavn as navn "
						+ "from Gruppe natural join GruppeØvelse natural join Øvelse "
						+ "where gruppeID = " + gruppeID + ";");
				while(result.next()){
					int id = result.getInt("id");
					if (!checkIntegerInList(printedØvelser, id)){
						String navn = result.getString("navn");
						System.out.println(id + " ... " + navn);
						printedØvelser.add(result.getInt("id"));
					}
	...
	private boolean checkIntegerInList(ArrayList<Integer> list, int num){
		return list.stream().anyMatch(listnum -> listnum == num);
	}
